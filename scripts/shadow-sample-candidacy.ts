...
for (const j of sample.slice(0, LIMIT)) {
 try {
  ...
  const disp = decide({ jobId: "shadow", companyId: "shadow", provider: j.provider, candidacy: c.verdict, candidacyReasonCode: c.reasonCodes[0] ?? null, hardMet: c.hardMet, hardTotal: c.hardTotal, eligibility: "ELIGIBLE", fit: null, baseSalaryMin: j.salaryMin ?? null, allFieldsConfident: true, blockedAnswers: 0, resumeClaimsAllGrounded: true, artifactValid: true, submittedToday: 0 } as Candidate, policy, switches);
